...
function anonymous(rankList) {
  var views = "";
  rankList.forEach((record) => {
    views += `
    <view class="record">
        <image class="avatar" src="${record.avatarUrl}"></image>
        <text class="nickname" value="${record.nickname}"></text>
    </view>
    `;
  });
  var out = `<scrollview id="container">${views}</scrollview>`;
  console.log(out);
  //   var arr1 = it.data;
  //   if (arr1) {
  //     var item,
  ...
  //         (index + 1) +
  //         '"></text> <image class="listHeadImg" src="' +
  //         item.avatarUrl +
  //         '"></image> <text class="listItemName" value="' +
  //         item.nickname +
  //         '"></text> </view> <text class="listItemScore" value="' +
  //         item.rankScore +
  //         ' 分"></text> </view> ';
  //     }
  //   }
  //   out +=
  //     ' </scrollview> <text class="listTips" value="仅展示前 ' +
  //     it.data.length +
  //     ' 位好友排名"></text> </view></view>';
  return out;
}
